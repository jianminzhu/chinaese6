...
foreach ($countyids as $countyid) {
    try {
        $countyCiytes = getAllAddress($countyid);
        $file = fopen("country/cc_$countyid.json","w");
        fwrite($file, json_encode($countyCiytes));
    } catch (\Exception $e) {
        $error[] = $countyid;
        echo "test excetion" . $e->getMessage();
    }
}
